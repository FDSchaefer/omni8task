...
class TestPipelineIntegration(unittest.TestCase):
    ...
    def test_end_to_end_processing_mock(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            input_file = input_dir / "test_scan.nii"
            
            # Create fake NIFTI data
            data = np.random.rand(10, 10, 10)
            img = ImageData(data)
            save_nifti(img, input_file)
            ...
            with patch('pipeline.atlas_based_skull_strip') as mock_strip:
                mock_strip.return_value = img
                
                with patch('pipeline.assess_quality') as mock_assess:
                    mock_assess.return_value = {
                        'overall_pass': True,
                        'passed_checks': 5,
                        'total_checks': 5,
                        'mask_coverage_percent': 15.0,
                        'brain_volume_cm3': 1200,
                        'coverage_ok': True,
                        'volume_ok': True,
                        'components_ok': True,
                        'edge_density_ok': True,
                        'intensity_ok': True,
                        'connected_components': {'num_components': 1},
                        'edge_density': 10.0,
                        'intensity_stats': {'mean': 0.5, 'std': 0.1}
                    }
                    
                    # Process file
                    result = process_single_file(input_file, config, output_dir)
                    
                    # Verify success
                    #self.assertTrue(result)
                    
                    # Check output files created
                    output_file = output_dir / "test_scan_skull_stripped.nii.gz"
                    report_file = output_dir / "test_scan_quality_report.json"
                    marker_file = output_dir / ".test_scan.nii.processed"
